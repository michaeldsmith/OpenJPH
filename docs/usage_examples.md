...
* Byte order on disk is independent of the machine's architecture; OpenJPH reads and writes the same bytes on little-endian and big-endian (e.g., s390x) machines.  Samples wider than one byte are little-endian in `.raw`/`.rawl` and `.yuv` files, and big-endian in 16-bit `.pgm`/`.ppm` files (per the netpbm specification).  `.pfm` files are written little-endian by default (the sign of the scale value in the header declares the byte order, and reading honors it, so `.pfm` files of either byte order can be read).  DPX files declare their byte order through the magic number, and TIFF byte order is handled by libtiff.
